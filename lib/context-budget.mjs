...
export function buildBudgetedPrompt(opts = {}) {
  ...
  // JD is always last and non-compressible — when provided. Pass an empty
  // string to exclude it from the system context entirely (the eval backends
  // send the JD only in the user turn to avoid billing it twice and keeping
  // untrusted content out of the scoring-rule system prompt).
  if (jdText) {
