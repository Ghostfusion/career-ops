...
| **Launchpad & Applied Toolkit** | Turn evaluations into action: `launchpad` tiers open rows ACT/PREP/HOLD/SKIP; `apply-queue` walks the active rows to submission; `screen-check` estimates ATS screen outcome; `expected-value` prioritizes prep; `watch-deadlines` flags reply/offer windows; `proof-point-bank` tracks the artifacts that unlock strong matches; `story-bank-seed` builds `interview-prep/story-bank.md` from every report; `warmup` gives a one-screen session digest; plus `negotiate`, `close-loop`, `rejection-reset`, `proof-portfolio`, `archetype-cv`, `strengths-audit`, `concentration`, `ingest-linkedin`, `salary-trend` (see `docs/SCRIPTS.md`) |
