...
try {
  ...
  if (capturedUrl === 'https://boards-api.greenhouse.io/v1/boards/acme/jobs?content=true&page=1&per_page=500' && capturedOpts?.redirect === 'error') {
    pass('greenhouse.fetch() hits the derived boards-api URL with content=true, pagination and redirect:"error" (SSRF guard)');
  } else {
  ...
  // and the pagination params must append after it.
  let pinnedUrl = null;
  ...
  if (pinnedUrl === 'https://boards-api.greenhouse.io/v1/boards/pinned/jobs?content=true&page=1&per_page=500')
    pass('greenhouse.fetch() does not duplicate an entry.api that already carries content=true and appends pagination');
